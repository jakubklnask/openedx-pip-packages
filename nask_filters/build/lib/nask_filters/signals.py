...
def auto_enroll_on_creation(sender, instance, created, **kwargs):
    ...
    """
    log.info(F"[NASK] Uruchomiono DjangoSignak AutoEnrollOnCreation")
    ...
    log.info(f"[NASK-SIGNAL] Wykryto utworzenie nowego użytkownika: {user.username}. Rozpoczynam auto-zapis.")
     ...
     # --- STAFF PASS ---
     # Admin (superuser) ma automatycznie dostęp do wszystkich kursów
     # Użytkownicy z is_staff (platform-wide instruktor) nie są niestety tak automatycznie zapisywani
     # Robimy to więc tutaj
    if user.is_staff:
        # Pobierz WSZYSTKIE kursy
        all_courses = list(CourseOverview.objects.all())
        for course in all_courses:
            try:
                log.info(f"[NASK-SIGNAL] Auto-zapisywanie nowego STAFF usera {user.username} na {course.id}")
