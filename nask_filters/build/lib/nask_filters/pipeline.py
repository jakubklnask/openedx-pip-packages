...
class AutoEnrollByCorpEmail(PipelineStep):
    ...
    def run_filter(self, user, *args, **kwargs):
        log.info(F"[NASK] Uruchomiono pipelinestep AutoEnrollByCorpEmail")
        # 0. Walidacja wstępna
        if not user.is_active:
            return {}
        
        # Pomijamy dla adminów
        if user.is_superuser:
            return {}
        
        # Staff też musimy automatycznie zapisać (na wypadek gdyby doszły jakieś kursy)
        # Tak samo jak w signals.py
        if user.is_staff:
